        ...
        implements Globals, Comparable<PositionData>
{
                                      ...
                                      AutoCrystal module)
    {
                ...
                && data.state.getBlock() != Blocks.OBSIDIAN)
        {
            if (!obby)
            {
                data.invalidReason = InvalidReason.BASE_BLOCK;
                return data;
            }

            if (!data.state.isReplaceable())
            {
                data.invalidReason = InvalidReason.BASE_OBBY_BLOCKED;
                return data;
            }

            if (checkEntities(data, pos, entities, 0, true, true, false))
            {
                data.invalidReason = InvalidReason.BASE_OBBY_BLOCKED;
                return data;
            }
        ...
        if (!upState.isAir())
        {
            ...
            {
                data.invalidReason = InvalidReason.UP_BLOCKED;
                return data;
            }
        }
        ...
        if (!newVer
                && !(upUpState = mc.world.getBlockState(up.up())).isAir())
        {
            ...
            {
                data.invalidReason = InvalidReason.UPUP_BLOCKED;
                return data;
            }
        }
        ...
        boolean blockedUp = checkEntities(
                data, up, entities, deathTime, false, false, checkLavaItems);
        boolean blockedUpUp = !newVerEntities
                && checkEntities(data, up.up(),
                                 entities, deathTime, false, false, checkLavaItems);
        if (blockedUp || blockedUpUp)
        {
            data.invalidReason = blockedUp
                    ? InvalidReason.ENTITY_BLOCKED_UP
                    : InvalidReason.ENTITY_BLOCKED_UP2;
            return data;
        }
                                         ...
                                         boolean lavaItems)
    {
        ...
        {
            boolean crystal = entity instanceof EndCrystalEntity;
            if (entity == null
                || spawning // && !entity.preventEntitySpawning
                || entity.isRemoved()
                || entity.isSpectator()
                || dead && EntityUtil.isDead(entity)
                || !crystal && EntityUtil.isDead(entity)
