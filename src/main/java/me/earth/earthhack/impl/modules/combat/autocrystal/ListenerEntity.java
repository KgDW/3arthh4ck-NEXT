...
final class ListenerEntity extends EntityS2CPacketListener
{
    ...
    public ListenerEntity(AutoCrystal module)
    {
            ...
            new ReceiveListener<>(EntityPositionS2CPacket.class, e ->
        {
            ...
            PlayerEntity p = getEntity(e.getPacket().entityId());
            if (p != null)
            {
                double x = e.getPacket().change().position().x;
                double y = e.getPacket().change().position().y;
                double z = e.getPacket().change().position().z;
                if (e.getPacket().relatives().contains(PositionFlag.X))
                {
                    x += p.getX();
                }

                if (e.getPacket().relatives().contains(PositionFlag.Y))
                {
                    y += p.getY();
                }

                if (e.getPacket().relatives().contains(PositionFlag.Z))
                {
                    z += p.getZ();
                }
                onEvent(p, x, y, z);
