        ...
        extends Finishable implements Globals
{
    ...
    private void runCalc()
    {
        ...
        if (placeCheck())
        {
            placeData = module.placeHelper.getData(all,
                                                   ...
                                                   blackList,
                                                   maxY);
            module.debug("place-data",
                    "PlaceData computed: data=" + placeData.getData().size()
                        + ", antiTotem=" + placeData.getAntiTotem().size()
                        + ", rayBypass=" + placeData.getRaytraceData().size()
                        + ", liquid=" + placeData.getLiquid().size()
                        + ", obby=" + placeData.getAllObbyData().size() + ".");
    ...
    protected boolean breakCheck()
    {
        ...
        {
            module.debug("break-disabled", "breakCheck=false: Break setting is disabled.");
            return false;
        }

        if (noBreak)
        {
            module.debug("break-nobreak", "breakCheck=false: noBreak flag active.");
            return false;
        }

        if (Managers.SWITCH.getLastSwitch() < module.cooldown.getValue())
        {
            module.debug("break-cooldown",
                    "breakCheck=false: switch cooldown not passed (" + module.cooldown.getValue() + "ms).");
            return false;
        }

        if (!module.breakTimer.passed(module.breakDelay.getValue()))
        {
            module.debug("break-delay",
                    "breakCheck=false: break delay not passed (" + module.breakDelay.getValue() + "ms).");
            return false;
        }
    ...
    protected boolean placeCheck()
    {
        ...
        {
            module.debug("place-sequential", "placeCheck=false: sequential helper blocking placement.");
            return false;
        }
        ...
        if (module.damageSync.getValue())
        {
                    ...
                    && c.isBreakConfirmed(module.breakConfirm.getValue())))
            {
                ...
                {
                    module.debug("place-damagesync",
                            "placeCheck=false: DamageSync confirmer waiting.");
                    return false;
                }
            }
        }

        if (count >= module.multiPlace.getValue())
        {
            module.debug("place-multiplace",
                    "placeCheck=false: count(" + count + ") >= multiPlace(" + module.multiPlace.getValue() + ").");
            return false;
        }

        if (Managers.SWITCH.getLastSwitch() < module.placeCoolDown.getValue())
        {
            module.debug("place-cooldown",
                    "placeCheck=false: place cooldown not passed (" + module.placeCoolDown.getValue() + "ms).");
            return false;
        }

        if (!module.place.getValue())
        {
            module.debug("place-disabled", "placeCheck=false: Place setting is disabled.");
            return false;
        }

        if (attacking && !module.multiTask.getValue())
        {
            module.debug("place-multitask",
                    "placeCheck=false: attacking and MultiTask disabled.");
            return false;
        }

        if (rotating && !module.rotate.getValue().noRotate(ACRotate.Place))
        {
            module.debug("place-rotating",
                    "placeCheck=false: currently rotating and place rotations required.");
            return false;
        }

        if (!module.placeTimer.passed(module.placeDelay.getValue()))
        {
            module.debug("place-delay",
                    "placeCheck=false: place delay not passed (" + module.placeDelay.getValue() + "ms).");
            return false;
        }

        if (noPlace)
        {
            module.debug("place-noplace", "placeCheck=false: noPlace flag active.");
            return false;
        }
    ...
    protected boolean check()
    {
        ...
        {
            module.debug("check-spectator", "check() aborted: player is spectator.");
            return true;
        }

        if (ANTISURROUND.returnIfPresent(AntiSurround::isActive, false))
        {
            module.debug("check-antisurround", "check() aborted: AntiSurround is active.");
            return true;
        }

        if (raw == null || entities == null)
        {
            module.debug("check-lists-null",
                    "check() aborted: raw/entities null. raw="
                        + (raw == null) + ", entities=" + (entities == null));
            return true;
        }

        if (module.stopWhenEating.getValue() && module.isEating())
        {
            module.debug("check-eating", "check() aborted: stopWhenEating active.");
            return true;
        }

        if (module.stopWhenEatingOffhand.getValue() && module.isEatingOffhand())
        {
            module.debug("check-eating-offhand", "check() aborted: stopWhenEatingOffhand active.");
            return true;
        }

        if (module.stopWhenMining.getValue() && module.isMining())
        {
            module.debug("check-mining", "check() aborted: stopWhenMining active.");
            return true;
        }
        ...
        if (all.isEmpty())
        {
            module.debug("check-no-targets",
                    "check() aborted: no valid targets. raw=" + raw.size()
        ...
        {
            module.debug("check-pingbypass", "check() aborted: ping bypass is active.");
            return true;
        }

        if (!module.attackMode.getValue().shouldCalc()
                && !module.canAutoSwitch()
                && !module.weaknessHelper.canSwitch()
                && !module.switching)
        {
            module.debug("check-no-calc",
                    "check() aborted: attackMode/canAutoSwitch/weakness disallow calculation.");
            return true;
        }

        boolean weakNoSwitch = module.weaknessHelper.isWeaknessed()
                && module.antiWeakness.getValue() == AntiWeakness.None;
        if (weakNoSwitch)
        {
            module.debug("check-weakness",
                    "check() aborted: weakness active and AntiWeakness=None.");
        }
