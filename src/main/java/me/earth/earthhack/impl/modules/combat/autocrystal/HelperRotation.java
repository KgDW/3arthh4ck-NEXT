...
public class HelperRotation implements Globals
{
                                        ...
                                        MutableWrapper<Boolean> attacked)
    {
        ...
        StopWatch timer = new StopWatch();
        MutableWrapper<Boolean> ended = new MutableWrapper<>(false);
        MutableWrapper<float[]> lastRotations = new MutableWrapper<>(null);
        return (x, y, z, yaw, pitch) ->
        {
            ...
            if (attacked.get())
            {
                ...
                if (ended.get()
                        && timer.passed(module.endRotations.getValue()))
                {
                    Entity crystal = module.getCrystal();
                    if (id == ID.get()
                        && (crystal == null || !crystal.equals(entity)))
                    {
                        module.rotation = null;
                    }

                    if (lastRotations.get() != null)
                    {
                        return lastRotations.get();
                    }
            ...
            float[] rotations =
                smoother.getRotations(entity, x, y, z,
                                      mc.player.getEyeHeight(mc.player.getPose()),
                                      module.height.getValue(),
                                      module.angle.getValue());
            lastRotations.set(rotations);
            return rotations;
        };
    }
        ...
        return () ->
        {
            ...
            if (!InventoryUtil.isHolding(Items.END_CRYSTAL))
            {
                ...
                if (!module.mainHand.getValue())
                {
                    ...
                    {
                        // Offhand logic is optional here, fallback to hotbar below.
                    }
                }
            }
            ...
            if (hand == null)
            {
                // Fallback to hotbar switching if offhand path failed.
                if (module.mainHand.getValue() || module.canAutoSwitch())
                {
            ...
            Vec3d hitVec = toAbsoluteHitVec(pos, ray.getPos());
            ...
            Locks.acquire(Locks.PLACE_SWITCH_LOCK, () ->
            {
                ...
                NetworkUtil.sendSequenced(seq -> new PlayerInteractBlockC2SPacket(finalHand, //TODO: check
                        new BlockHitResult(finalHitVec, finalRay.getSide(),
    ...
    public Runnable post(Entity entity, MutableWrapper<Boolean> attacked)
    {
        return () ->
        {
            ...
            boolean breakingRotations = !module.rotate.getValue().noRotate(ACRotate.Break)
                    && !module.isNotCheckingRotations();
            boolean legit = RotationUtil.isLegit(entity);
            if (!legit && module.hasRenderRotations())
            {
                legit = RotationUtil.isLegit(entity,
                                             module.getRenderRotationYaw(),
                                             module.getRenderRotationPitch(),
                                             entity);
            }
        ...
        return () ->
        {
            ...
            Locks.acquire(Locks.PLACE_SWITCH_LOCK, () ->
            {
                ...
                for (Ray ray : data.getPath())
                {
                    ...
                    BlockPos clickPos = ray.getResult().getBlockPos();
                    Vec3d clickVec = toAbsoluteHitVec(clickPos, ray.getResult().getPos());

                    NetworkUtil.sendSequenced(seq ->
                            new PlayerInteractBlockC2SPacket(hand,
                                    new BlockHitResult(clickVec, ray.getFacing(), clickPos, false), seq));
    ...
    private static Vec3d toAbsoluteHitVec(BlockPos pos, Vec3d hitVec)
    {
        if (hitVec == null)
        {
        ...
        return hitVec.add(pos.getX(), pos.getY(), pos.getZ());
    }
