    ...
    public void invoke(MotionUpdateEvent event)
    {
                ...
                && mc.player.isOnGround())
        {
            mc.player.setVelocity(
                    mc.player.getVelocity().multiply(0.0, 1.0, 0.0));
                ...
                Locks.acquire(Locks.PLACE_SWITCH_LOCK, () ->
                {
                    ...
                    BlockHitResult result =
                            RayTraceUtil.getBlockHitResult(module.rotations[0],
                                                           module.rotations[1]);
                    BlockPos on = module.pos.offset(module.facing);
                    Direction side = module.facing.getOpposite();
                    if (result.getType() != HitResult.Type.BLOCK)
                    {
                        result = new BlockHitResult(on.toCenterPos(),
                                                    side,
                                                    on,
                                                    false);
                    }
                    ...
                    {
                        result = new BlockHitResult(result.getPos(),
                                                    side,
                                                    on,
                                                    false);
                    }
