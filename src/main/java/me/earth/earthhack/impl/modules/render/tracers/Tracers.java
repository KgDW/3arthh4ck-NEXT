...
public class Tracers extends EntityTypeModule
{
    ...
    public boolean isValid(Entity entity)
    {
        ...
        if (distanceSq < MathUtil.square(minRange.getValue())
                || distanceSq > MathUtil.square(maxRange.getValue()))
        {
            return false;
        }
        ...
        if (items.getValue() && entity instanceof ItemEntity)
        {
            return true;
        }

        if (players.getValue() && entity instanceof PlayerEntity player
                && (invisibles.getValue() || !player.isInvisible())
                && (friends.getValue() || !Managers.FRIENDS.contains(player)))
        {
            return true;
        }
