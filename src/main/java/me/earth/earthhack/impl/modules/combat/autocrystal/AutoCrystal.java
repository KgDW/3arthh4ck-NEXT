...
public class AutoCrystal extends Module
{
    ...
    protected final Setting<Boolean> newVer =
            register(new BooleanSetting("1.13+", true))
                .setComplexity(Complexity.Medium);
    ...
    protected final Setting<Boolean> slide =
            register(new BooleanSetting("Slide", true))
                .setComplexity(Complexity.Medium);
    protected final Setting<Boolean> smoothSlide =
            register(new BooleanSetting("SmoothenSlide", true))
                .setComplexity(Complexity.Medium);
    protected final Setting<Double> slideTime =
            register(new NumberSetting<>("Slide-Time", 400.0, 1.0, 1000.0))
                .setComplexity(Complexity.Expert);
    protected final Setting<Boolean> zoom =
            register(new BooleanSetting("Zoom", true))
                .setComplexity(Complexity.Medium);
    protected final Setting<Double> zoomTime =
            register(new NumberSetting<>("Zoom-Time", 450.0, 1.0, 1000.0))
                .setComplexity(Complexity.Medium);
    ...
    protected final Setting<RenderDamagePos> renderDamage =
            register(new EnumSetting<>("DamageRender", RenderDamagePos.Inside))
                .setComplexity(Complexity.Medium);
    ...
    protected final Setting<Boolean> debug =
            register(new BooleanSetting("Debug", false))
                .setComplexity(Complexity.Dev);
    protected final Setting<Integer> debugDelay =
            register(new NumberSetting<>("DebugDelay", 750, 0, 5000))
                .setComplexity(Complexity.Dev);
    protected final Setting<Boolean> tickFallback =
            register(new BooleanSetting("TickFallback", true))
                .setComplexity(Complexity.Expert);
    ...
    private final Map<String, Long> debugTimestamps =
            new ConcurrentHashMap<>();
    ...
    protected BlockPos lastZoomPos;
    protected final StopWatch pullTimer = new StopWatch();
    protected final StopWatch motionEventTimer = new StopWatch();
    ...
    protected volatile float renderRotationYaw;
    protected volatile float renderRotationPitch;
    protected volatile long renderRotationTime;
    protected BlockPos slidePos;
    protected double slideX;
    protected double slideY;
    protected double slideZ;
    ...
    public void setRenderPos(BlockPos pos, String text) {
        renderTimer.reset();
        if (pos != null && !pos.equals(renderPos)) {
            if (smoothSlide.getValue() && renderPos != null) {
                double rawFactor = Math.min(1.0, slideTimer.getTime() / Math.max(1.0, slideTime.getValue()));
                double t = 1.0 - rawFactor;
                double factor = 1.0 - t * t * t;
                if (slidePos != null && factor < 1.0) {
                    slideX = slideX + (renderPos.getX() - slideX) * factor;
                    slideY = slideY + (renderPos.getY() - slideY) * factor;
                    slideZ = slideZ + (renderPos.getZ() - slideZ) * factor;
                } else {
                    slideX = renderPos.getX();
                    slideY = renderPos.getY();
                    slideZ = renderPos.getZ();
                }
        ...
        if (pos != null && renderPos == null && zoom.getValue()) {
            zoomingIn = true;
            zoomTimer.reset();
        }
    ...
    public BlockPos getRenderPos() {
        if (renderTimer.passed(renderTime.getValue())) {
            if (renderPos != null && zoom.getValue()) {
                lastZoomPos = renderPos;
                zoomingIn = false;
                zoomTimer.reset();
            }
            renderPos = null;
    ...
    /**
     * Treat Bind mode without a configured key as always-on switching.
     * This avoids a "does nothing" state on fresh configs.
     */
    public boolean canAutoSwitch()
    {
        AutoSwitch mode = autoSwitch.getValue();
        if (mode == AutoSwitch.Always)
        {
            return true;
        }

        if (mode == AutoSwitch.Bind)
        {
            return switching || switchBind.getValue().getKey() == Bind.none().getKey();
        }
    ...
    protected void reset()
    {
        ...
        slideY = 0;
        slideZ = 0;
        renderRotationYaw = 0.0f;
        renderRotationPitch = 0.0f;
        renderRotationTime = 0L;
        lastZoomPos = null;
        zoomingIn = false;
        rotation = null;
        switching = false;
    ...
    public void setRenderRotations(float yaw, float pitch)
    {
        this.renderRotationYaw = yaw;
        this.renderRotationPitch = pitch;
        this.renderRotationTime = System.currentTimeMillis();
    }

    public boolean hasRenderRotations()
    {
        long time = renderRotationTime;
        return time != 0L
                && System.currentTimeMillis() - time
                    <= Math.max(500L, endRotations.getValue() + 50L);
    }

    public float getRenderRotationYaw()
    {
        return renderRotationYaw;
    }

    public float getRenderRotationPitch()
    {
        return renderRotationPitch;
    }

    public boolean shouldUseTickFallback()
    {
        return tickFallback.getValue()
                && motionEventTimer.passed(250L);
    }
    ...
    public void debug(String key, String message)
    {
        ...
        String debugKey = key == null ? "generic" : key;
        Long last = debugTimestamps.put(debugKey, now);
        if (last != null && now - last < debugDelay.getValue())
        {
            return;
        }

        String line = "[AC-DBG] " + message;
        Earthhack.getLogger().info(line);
        ChatUtil.sendMessageScheduled(line, "autocrystal-debug-" + debugKey);
    }
    ...
    public boolean isOutsidePlaceRange(BlockPos pos) {
        ...
        double y = Managers.POSITION.getY()
                + (placeRangeEyes.getValue() ? player.getEyeHeight(player.getPose()) : 0);
