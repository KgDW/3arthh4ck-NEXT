...
public class Render2DUtil implements Globals {
    private static final Deque<double[]> SCISSOR_STACK = new ArrayDeque<>();
    ...
    public static void drawPlayerFace(DrawContext context, PlayerEntity player, int x, int y, int width, int height) {
        if (player != null && mc.player != null) {
            ...
            context.drawTexture(RenderLayer::getGuiTextured, resourceLocation, x, y, 8, 8, width, height, 64, 64);
    ...
    public static void scissor(float x, float y, float x1, float y1) {
        if (Managers.TEXT.usingCustomFont()) {
            TextRenderer.FONTS.pushClip(x, y, x1, y1);
        }

        float ox = Click.getRenderOffsetX();
        float oy = Click.getRenderOffsetY();
        x += ox;
        x1 += ox;
        y += oy;
        y1 += oy;
        ...
        double ex = Math.max(x, x1);
        double ey = Math.max(y, y1);

        if (!SCISSOR_STACK.isEmpty()) {
            double[] parent = SCISSOR_STACK.peek();
        ...
        double[] rect = new double[]{sx, sy, ex, ey};
    ...
    public static void disableScissor() {
        ...
        applyScissor(SCISSOR_STACK.peek());
    }

    private static void applyScissor(double[] rect) {
        double sx = rect[0];
        double sy = rect[1];
        double ex = rect[2];
        double ey = rect[3];
        double w = Math.max(0.0, ex - sx);
        double h = Math.max(0.0, ey - sy);
        ...
        int pw = (int) Math.round(w * f);
        int ph = (int) Math.round(h * f);
