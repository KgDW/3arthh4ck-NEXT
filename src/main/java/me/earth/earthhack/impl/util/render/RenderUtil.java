...
public class RenderUtil implements Globals {
    ...
    // todo vertexbuffers
    private static final VertexBuffer BLOCK_FILL_BUFFER = new VertexBuffer(GlUsage.STATIC_WRITE);
    private static final VertexBuffer BLOCK_OUTLINE_BUFFER = new VertexBuffer(GlUsage.STATIC_WRITE);
    ...
    private static final Matrix4f WORLD_POSITION_MATRIX = new Matrix4f();
    ...
    {
        WORLD_POSITION_MATRIX.set(positionMatrix);
        WORLD_PROJECTION_MATRIX.set(projectionMatrix);
    }

    public static Vector4f projectToScreen(double x, double y, double z)
    {
        ...
        WORLD_POSITION_MATRIX.transform(clip);
        WORLD_PROJECTION_MATRIX.transform(clip);
        ...
        return new Vector4f(screenX, screenY, ndcZ, clip.w);
    }
    ...
    public static void fillOutline(MatrixStack matrix, Box bb, Color color)
    {
        if (bb != null)
        {
            ...
            VertexRendering.drawBox(matrix, bufferBuilder, bb.minX, bb.minY, bb.minZ, bb.maxX, bb.maxY, bb.maxZ, red, green, blue, alpha);
