...
public class PacketUtil implements Globals
{
                                     ...
                                     boolean event)
    {
        ...
        float yaw   = noRotate ? entity.getYaw() : packet.change().yaw();
        float pitch = noRotate ? entity.getPitch() : packet.change().pitch();

        entity.setPos(x, y, z);
        entity.setPitch(pitch);
        entity.setYaw(yaw);

        Packet<?> confirm = new TeleportConfirmC2SPacket(packet.teleportId());
    ...
    {
        return new PlayerMoveC2SPacket.OnGroundOnly(onGround, false);
    }
    ...
    public static PlayerMoveC2SPacket rotation(float yaw,
                                         ...
                                         boolean onGround)
    {
        return new PlayerMoveC2SPacket.LookAndOnGround(yaw, pitch, onGround, false);
    }
