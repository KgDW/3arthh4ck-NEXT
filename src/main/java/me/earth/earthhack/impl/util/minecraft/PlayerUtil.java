...
public class PlayerUtil implements Globals {
    ...
    public static OtherClientPlayerEntity createFakePlayerAndAddToWorld(GameProfile profile) {
        return createFakePlayerAndAddToWorld(profile, false);
    }

    public static OtherClientPlayerEntity createFakePlayerAndAddToWorld(GameProfile profile,
                                                                        boolean totem) {
        OtherClientPlayerEntity fakePlayer = createFakePlayer(profile, totem);
        int randomID = -1000;
    ...
    {
        return createFakePlayer(profile, false);
    }

    public static OtherClientPlayerEntity createFakePlayer(GameProfile profile,
                                                           boolean totem)
    {
        OtherClientPlayerEntity fakePlayer = totem
                ? new EntityPlayerPop(mc.world, profile)
                : new OtherClientPlayerEntity(mc.world, profile);
    ...
    public static void removeFakePlayer(PlayerEntity fakePlayer) {
        mc.execute(() -> {
            ...
            fakePlayer.discard(); // setDead might be overridden
            if (mc.world != null) {
